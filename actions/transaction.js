...
export async function createTransaction(data) {
  try {
    ...
    return { success: true, data: serialeAmount(transaction) };
  } catch (error) {
    console.log(error);
...
//Helper function to calculate next recurring data
...
export async function scanReceipt(file) {
  try {
    ...
    const prompt = `Analyze this receipt image and extract the following information in JSON format:
                       ...
                       "description":"string",   (if not give empty string "")
                       "merchantName":"string",   (if not give empty string "")
                       "category":"string", 
                       }
...
export async function getTransaction(id){
   ...
   if(!user) throw new Error("User not found!");
    ...
    return serialeAmount(transaction);
}
